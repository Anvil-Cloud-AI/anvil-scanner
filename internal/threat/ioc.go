...
// sudoUserRE validates that SUDO_USER is a plain Unix username (no injection risk).
var sudoUserRE = regexp.MustCompile(`^[a-zA-Z0-9_.\-]{1,64}$`)
		...
		if m := authSudoRE.FindStringSubmatch(line); m != nil {
			...
			if os.Getuid() == 0 {
				if sudoUser := os.Getenv("SUDO_USER"); sudoUser != "" {
					if sudoUserRE.MatchString(sudoUser) {
