...
- Dependency CVEs in anvil-scanner's own Go modules (caught by govulncheck in CI, not at runtime on the target host).
...
### anvil-scanner's own attack surface
- Command injection via subprocess — prevented by explicit argument lists everywhere; no shell interpolation.
- SSRF via the Ollama integration — prevented by a loopback-only URL allowlist and SSRF-safe transport.
- XSS in HTML reports — prevented by `html.EscapeString` on every external-data field.
- Path traversal in backup / restore — prevented by source-inside-session and destination-in-managed-prefix validation.
- TOCTOU on config-file writes — prevented by temp-file + rename; never direct overwrite.
- Memory exhaustion — all file reads use `io.LimitReader`; subprocess output capped at 1 MB.
- Secret exposure — secrets never written to plaintext on disk; OS keyring, AES-256-GCM encrypted file, or env var only.

Every push gates on gosec, govulncheck, staticcheck, and semgrep with zero unreviewed high-severity findings as the release bar. See [scanners.md](scanners.md).
...
govulncheck catches CVEs in what we pin. It does not catch typosquats, compromised upstream accounts that haven't been reported, or build-time tampering. SBOMs and signed release artifacts narrow this, but we do not claim full supply-chain defense.
...
- **The Go standard library.** We trust Go's stdlib behaviors — `crypto/tls` returns a properly configured context with default certificate verification, `os/exec.Command(binary, args...)` doesn't interpret a shell, `html.EscapeString` escapes the five dangerous characters.
